...
guidance.llm = guidance.llms.OpenAI("gpt-3.5-turbo-16k", api_key=open_ai_key)

llm = ChatOpenAI(temperature=0, model="gpt-3.5-turbo-0613",request_timeout=120)
...
transform_dic_str = create_extraction_chain(
    schema_dic_str,
    llm,
)
...
def str2lst(str):
    ...
    assert len(quotes_indices) == 6, "The number of double quotes in the string is not equal to 6!"
    # extract the strings between the double quotes
    sentences = [str[quotes_indices[i]+1:quotes_indices[i+1]] for i in range(0,len(quotes_indices),2)]
    return sentences 
...
def lambda_handler(event, context):
    ...
    challenge_str = output["challenge_json"]
    ...
    print("\n")
    # try:
    #     #remove \n and space in the string
    #     preferences = preferences.replace("\n", "")
    #     preferences = preferences.replace(" ", "") 
    #     preferences = dic2list(eval(preferences[1:-1]))
    # except Exception as e:
    #     preferences = dic2list(transform_dic_str.run(preferences))
    ...
    print(growth_plan)
    print("\n")
    # try: sen1<space>sen2<space>sen3
    #     #remove \n and space in the string
    #     growth_plan = growth_plan.replace("\n", "")
    #     growth_plan = growth_plan.replace(" ", "")
    #     growth_plan = dic2list(eval(growth_plan[1:-1]))
    # except Exception as e:
    #     growth_plan = dic2list(transform_dic_str.run(growth_plan))

    #Raise error if Error happen because growth_plane is not in expected format
    try : 
